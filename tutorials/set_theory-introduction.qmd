...
A set is a collection of distinct objects or elements ($e$). We reference a set by listing all its elements.  
*For example, $S = \{cat, \;mouse, \;dog \}$ or $S = \{1, 2, 3, 4, 5, 6 \}$ to describe all the possible distinct outcomes when we roll a dice and observe its upper face.*     
