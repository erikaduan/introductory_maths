...
Vectors are an important way to represent object position in m-th
dimensional space. Vector operations enable us to transform vectors in
useful ways.
...
-   Vector addition - when two vectors of the same dimensions are added
    to form a new vector (with a new length and direction). This is
    equivalent to creating a new position in n-th dimensional space.  
...
as long as a consistent solution to the linear system exists.
