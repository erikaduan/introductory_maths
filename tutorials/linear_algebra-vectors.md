...
vectors to find solutions to consistent linear systems.
...
-   Vector multiplication - used to calculate how far apart two vectors
    ...
    length of one vector along the axis of the other vector.
...
usually produces a new vector with a new length and direction in
![\\mathbb{R}^m](https://latex.codecogs.com/svg.format?%5Cmathbb%7BR%7D%5Em "\mathbb{R}^m").
...
![{\\vec v_1, \\vec v_2, \\cdots, \\vec v_p}](https://latex.codecogs.com/svg.format?%7B%5Cvec%20v_1%2C%20%5Cvec%20v_2%2C%20%5Ccdots%2C%20%5Cvec%20v_p%7D "{\vec v_1, \vec v_2, \cdots, \vec v_p}")
which span
![\\vec b](https://latex.codecogs.com/svg.format?%5Cvec%20b "\vec b") do
not need to be linearly independent. If the number of vectors exceeds
the number of linear equations
i.e. ![p\>m](https://latex.codecogs.com/svg.format?p%3Em "p>m") and
there are more columns than rows in the augmented matrix form of
![A\\vec cx = \\vec b](https://latex.codecogs.com/svg.format?A%5Cvec%20cx%20%3D%20%5Cvec%20b "A\vec cx = \vec b"),
the set of vectors are always linearly dependent.
...
that the vectors
![\\vec v_1, \\vec v_2, \\cdots, \\vec v_n](https://latex.codecogs.com/svg.format?%5Cvec%20v_1%2C%20%5Cvec%20v_2%2C%20%5Ccdots%2C%20%5Cvec%20v_n "\vec v_1, \vec v_2, \cdots, \vec v_n")
are linearly dependent. The geometric intuition for this is that
multiple non-trivial linear combinations of
...
![\\{\\vec v_1, \\vec v_2, \\cdots, \\vec v_p \| \\vec v \\in \\mathbb{R}^m\\}](https://latex.codecogs.com/svg.format?%5C%7B%5Cvec%20v_1%2C%20%5Cvec%20v_2%2C%20%5Ccdots%2C%20%5Cvec%20v_p%20%7C%20%5Cvec%20v%20%5Cin%20%5Cmathbb%7BR%7D%5Em%5C%7D "\{\vec v_1, \vec v_2, \cdots, \vec v_p | \vec v \in \mathbb{R}^m\}").
Let a new vector
...
vectors. The set
![\\{\\vec v_1, \\vec v_2, \\cdots, \\vec v_p, \\vec w\\}](https://latex.codecogs.com/svg.format?%5C%7B%5Cvec%20v_1%2C%20%5Cvec%20v_2%2C%20%5Ccdots%2C%20%5Cvec%20v_p%2C%20%5Cvec%20w%5C%7D "\{\vec v_1, \vec v_2, \cdots, \vec v_p, \vec w\}")
is only linearly dependent if
![\\vec w \\in Span\\{\\vec v_1, \\vec v_2, \\cdots, \\vec v_p\\}](https://latex.codecogs.com/svg.format?%5Cvec%20w%20%5Cin%20Span%5C%7B%5Cvec%20v_1%2C%20%5Cvec%20v_2%2C%20%5Ccdots%2C%20%5Cvec%20v_p%5C%7D "\vec w \in Span\{\vec v_1, \vec v_2, \cdots, \vec v_p\}").
...
exists which transforms A to B and A to C. As EROs are reversible, a
finite sequence of EROs exists which transforms B to C and B and C are
therefore row equivalent.
...
must contain the same number of pivot columns and non-pivot columns.
...
is therefore linearly independent if removing a vector decreases the
span of the vector set. A set of linearly independent vectors can also
be extended to form a larger set of linearly independent vectors if the
new vector is not in the span of the original set.

# Vector subspaces

A vector subspace is simply a closed vector space that exists inside
![\\mathbb{R}^m](https://latex.codecogs.com/svg.format?%5Cmathbb%7BR%7D%5Em "\mathbb{R}^m").
For example, take the set of vectors
...
where each vector has dimensions
![m \\times 1](https://latex.codecogs.com/svg.format?m%20%5Ctimes%201 "m \times 1").

![Span\\{\\vec v_1, \\vec v_2, \\vec v_3\\}](https://latex.codecogs.com/svg.format?Span%5C%7B%5Cvec%20v_1%2C%20%5Cvec%20v_2%2C%20%5Cvec%20v_3%5C%7D "Span\{\vec v_1, \vec v_2, \vec v_3\}")
is a 3D vector subspace, denoted as subspace H where
![H \\in \\mathbb{R}^m](https://latex.codecogs.com/svg.format?H%20%5Cin%20%5Cmathbb%7BR%7D%5Em "H \in \mathbb{R}^m"),
...
-   ![Span(\\vec v_1, \\vec v_2, \\vec v_3)](https://latex.codecogs.com/svg.format?Span%28%5Cvec%20v_1%2C%20%5Cvec%20v_2%2C%20%5Cvec%20v_3%29 "Span(\vec v_1, \vec v_2, \vec v_3)")
    ...
    ![\\vec 0](https://latex.codecogs.com/svg.format?%5Cvec%200 "\vec 0").  
-   If vectors
    ![\\vec u_1, \\vec u_2](https://latex.codecogs.com/svg.format?%5Cvec%20u_1%2C%20%5Cvec%20u_2 "\vec u_1, \vec u_2")
    are in subspace H, so is
    ![\\vec u_1 + \\vec u_2](https://latex.codecogs.com/svg.format?%5Cvec%20u_1%20%2B%20%5Cvec%20u_2 "\vec u_1 + \vec u_2").
    This is also known as being closed under vector addition.  
-   If vector
    ![\\vec u_1](https://latex.codecogs.com/svg.format?%5Cvec%20u_1 "\vec u_1")
    is in the subspace, so is
    ![c \\vec u_1](https://latex.codecogs.com/svg.format?c%20%5Cvec%20u_1 "c \vec u_1")
    where
    ![c \\in \\mathbb{R}](https://latex.codecogs.com/svg.format?c%20%5Cin%20%5Cmathbb%7BR%7D "c \in \mathbb{R}").
    This is also known as being closed under scalar multiplication.

Therefore, any subspace in
![\\mathbb{R}^m](https://latex.codecogs.com/svg.format?%5Cmathbb%7BR%7D%5Em "\mathbb{R}^m")
...
where
![\\{\\vec v_1, \\cdots, \\vec v_p \\}](https://latex.codecogs.com/svg.format?%5C%7B%5Cvec%20v_1%2C%20%5Ccdots%2C%20%5Cvec%20v_p%20%5C%7D "\{\vec v_1, \cdots, \vec v_p \}")
can be a set of either linearly dependent or linearly independent column
vectors with dimensions
![m \\times 1](https://latex.codecogs.com/svg.format?m%20%5Ctimes%201 "m \times 1").

<img src="../figures/linear_systems-vector_subspaces.svg" width="80%" style="display: block; margin: auto;" />

<img src="../figures/linear_systems-vector_subspace_examples.svg" width="80%" style="display: block; margin: auto;" />
...
-   The
    ![Span\\{\\vec v_1, \\cdots, \\vec v_p \\}](https://latex.codecogs.com/svg.format?Span%5C%7B%5Cvec%20v_1%2C%20%5Ccdots%2C%20%5Cvec%20v_p%20%5C%7D "Span\{\vec v_1, \cdots, \vec v_p \}")
    forms subspace H
    i.e. ![\\{\\vec v_1, \\cdots, \\vec v_p \\} \\in \\mathbb{R^m}](https://latex.codecogs.com/svg.format?%5C%7B%5Cvec%20v_1%2C%20%5Ccdots%2C%20%5Cvec%20v_p%20%5C%7D%20%5Cin%20%5Cmathbb%7BR%5Em%7D "\{\vec v_1, \cdots, \vec v_p \} \in \mathbb{R^m}")
    and
    ![H \\in \\mathbb{R^m}](https://latex.codecogs.com/svg.format?H%20%5Cin%20%5Cmathbb%7BR%5Em%7D "H \in \mathbb{R^m}").  
...
![\\mathbb{R}^{n}](https://latex.codecogs.com/svg.format?%5Cmathbb%7BR%7D%5E%7Bn%7D "\mathbb{R}^{n}")
where
![\\vec e_1 = \\begin{bmatrix}1&0&\\cdots&0\\end{bmatrix}^T](https://latex.codecogs.com/svg.format?%5Cvec%20e_1%20%3D%20%5Cbegin%7Bbmatrix%7D1%260%26%5Ccdots%260%5Cend%7Bbmatrix%7D%5ET "\vec e_1 = \begin{bmatrix}1&0&\cdots&0\end{bmatrix}^T")
with dimensions
