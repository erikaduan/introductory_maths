...
-   Vector addition - when two vectors with the same dimensions are
    ...
    This is equivalent to creating a new position in n-th dimensional
    space.  
...
![x_1\\vec a_1 + x_2\\vec a_2 + \\cdots + x_n\\vec a_n = \\vec b](https://latex.codecogs.com/svg.format?x_1%5Cvec%20a_1%20%2B%20x_2%5Cvec%20a_2%20%2B%20%5Ccdots%20%2B%20x_n%5Cvec%20a_n%20%3D%20%5Cvec%20b "x_1\vec a_1 + x_2\vec a_2 + \cdots + x_n\vec a_n = \vec b")
and then solving for the vector coefficients
...
![x_1 = x_2 = \\cdots = x_n](https://latex.codecogs.com/svg.format?x_1%20%3D%20x_2%20%3D%20%5Ccdots%20%3D%20x_n "x_1 = x_2 = \cdots = x_n"),
...
set. A set of linearly independent vectors can also be extended to form
a larger set of linearly independent vectors if the new vector is not in
...
If matrix B and C are row equivalent, their columns must satisfy the
same linear dependence equations and B and C must contain the same
number of pivot columns.

<img src="../figures/linear_systems-reduced_echelon_form_proof.svg" width="80%" style="display: block; margin: auto;" />

-   Pivot columns in a reduced echelon form matrix cannot be written as
    a linear combination of all pivot columns to its left.  
-   Non-pivot columns in a reduced echelon form matrix can be written
    using a unique linear combination of all pivot columns to its left.
...
Any subspace of
![\\mathbb{R}^n](https://latex.codecogs.com/svg.format?%5Cmathbb%7BR%7D%5En "\mathbb{R}^n")
is therefore also the span of a finite set of vectors
...
We are interested in describing subspaces of
![\\mathbb{R}^{n}](https://latex.codecogs.com/svg.format?%5Cmathbb%7BR%7D%5E%7Bn%7D "\mathbb{R}^{n}")
as the span of a set of vectors
![\\{\\vec v_1, \\cdots, \\vec v_p \\}](https://latex.codecogs.com/svg.format?%5C%7B%5Cvec%20v_1%2C%20%5Ccdots%2C%20%5Cvec%20v_p%20%5C%7D "\{\vec v_1, \cdots, \vec v_p \}").
Defining a vector subspace allows us to narrow the search space for an
...
-   The set of vectors
    ![\\{\\vec v_1, \\cdots, \\vec v_p \\}](https://latex.codecogs.com/svg.format?%5C%7B%5Cvec%20v_1%2C%20%5Ccdots%2C%20%5Cvec%20v_p%20%5C%7D "\{\vec v_1, \cdots, \vec v_p \}")
    are linearly independent and spans subspace H.  
-   The set of vectors
    ![\\{\\vec v_1, \\cdots, \\vec v_p \\}](https://latex.codecogs.com/svg.format?%5C%7B%5Cvec%20v_1%2C%20%5Ccdots%2C%20%5Cvec%20v_p%20%5C%7D "\{\vec v_1, \cdots, \vec v_p \}")
    span subspace H.  
