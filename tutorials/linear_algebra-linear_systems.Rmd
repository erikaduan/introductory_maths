...
In linear algebra, we solve for a solution set $S = \{s_1, s_2, \cdots\, s_n\}$ to unknown variables $\{x_1, x_2, \cdots\, x_n\}$ given that multiple observations or constraints are described. Each observation or constraint is incorporated as an additional linear equation containing different scalars of the same unknown variables. A set of linear equations that must be simultaneously solved is called a **linear system**.      

In statistics, a linear system is used to describe multiple observations of a phenomenon and $X$ and $\vec y$ are treated as random variables. Within this linear system, given observed values of $X$ and $\vec y$, we solve for $\vec b$ where $X \vec b + \vec \epsilon = \vec y$. $X$ and $\vec y$ are random variables because we attribute a component of randomness to observed $X$ and $\vec y$ values. We are therefore never interested in modelling a single solution to our linear system i.e. to generate a perfect curve which fits through all points of $X$ and $\vec y$. In statistics, the latter task is known as model overfitting.    

In contrast, in mathematical modelling, a linear system contains multiple fixed observations and we solve for $(x_1, x_2, ..., x_n)$ or $\vec x$ where $\vec a_1x_1  + \vec a_2x_2 + \cdots + \vec a_nx_n = \vec b$ or $A\vec x = \vec b$ and $a_{i,j}, b \in \mathbb{R}$. The linear system is true when a set of values $(c_1, c_2, ..., c_n)$ substitutes for $(x_1, x_2, ..., x_n)$.   
...
+ The linear system is consistent and has infinite solutions (or the solution set has infinitely many elements) i.e. $S = \{(c_1, c_2, x_3 - 2) | x_3 \in \mathbb{R}\}$ where $c_1, c_2$ are fixed scalars and $x_3$ represents any real number.    
...
A linear system can be represented in matrix form, through an augmented matrix for linear systems with form $A\vec x = \vec b$ and through a coefficient matrix for linear systems with form $A\vec x = \vec 0$.   
...
2. Further reducing the matrix to its reduced echelon form, where the solution for each variable is obvious i.e. $x_1 = c_1, x_2 = c_2, \cdots, x_n = c_n$.           
...
A homogeneous linear system $A\vec x=\vec0$ is linearly independent if it only has one trivial solution i.e. only the solution $x_1 = x_2 = \cdots = x_n = 0$ exists. The reduced echelon form of matrix $A$ posits that a homogeneous linear system with a single trivial solution does not contain any free variables. Homogeneous linear systems containing free variables are therefore not linearly independent.    

If a homogeneous linear system has infinite solutions, its augmented matrix form contains at least one free variable and the system is linearly dependent.   
