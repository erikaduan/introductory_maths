...
We can therefore re-write infinite solutions to a non-homogeneous linear system $A\vec x=\vec b$ as the addition of a positional vector $\vec p$ to the infinite solution set of a homogeneous linear system $A\vec x=\vec 0$.    
...
When an infinite solution is expressed as a parametric vector form, we can form some geometric intuition about the space that it occupies. For an infinite solution, the solution is an $\mathbb{R}^{n-m}$ object which intersects with the point represented by $\vec p$.  
...
A homogeneous linear system $A\vec v=\vec0$ is linearly independent if it only has one trivial solution i.e. there is no combination of $x_1 \vec v_1 + \cdots + x_n \vec v_n$ equal to 0 except $x_1 = x_2 = \cdots = x_n = 0$. By definition, a homogeneous linear system with a single trivial solution does not contain any free variables. Homogeneous linear systems containing free variables are therefore not linearly independent.   

If a homogeneous linear system has infinite solutions, at least one free variable is present and the system is therefore linearly dependent. Alternatively, you can think of linearly dependent homogeneous linear systems as systems where $x_1 \vec v_1 + \cdots + x_n \vec v_n = 0$ contains a solution with parametric form $\vec 0 + x_i\vec v_i$.        

```{r, echo=FALSE, results='markup'}
knitr::include_graphics("../figures/linear_systems-linear_dependence_a.svg")
```

Linear independence and linear dependence are easier to conceptualise when we view the coefficient matrix A as a row of column vectors $A = \begin{bmatrix} \vec a_1 & \vec a_2 & \cdots & \vec a_n \end{bmatrix}$.    
