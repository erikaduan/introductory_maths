...
What happens when we have a collection of linear equations, or a linear system? A linear system is used to contain multiple observations of a phenomenon i.e. for modelling purposes.   
...
+ The linear system is consistent and has infinite solution (or the solution set has infinitely many elements) i.e. $\{(a, b, x_3 - 2) | x_3 \in \mathbb{R}\}$. This occurs by default when you have less observations than variables i.e. the $n<p$ [problem](https://stats.stackexchange.com/questions/385711/what-is-the-problem-with-p-n) in statistics.      

In 2D, we can see that solutions have two properties:  
+ Solutions can be solved using a consistent mathematical approach.  
+ Solutions have a geometric intuition. For example in 2D, solutions can be represented as two lines which never intersect, two lines which intersect once, or two lines superimposed on each other i.e. multiple 'intersections'.    
...
+ All non-zero rows are above rows of all zeros. Rows of all zeros are used to satisfy matrix representations of situations where $n<p$.    
+ Each leading entry (or pivot column) of a row is located to the left of the leading entry of the row below it.    
...
+ [YouTube video series](https://www.youtube.com/watch?v=ZKUqtErZCiU&list=PLHXZ9OQGMqxfUl0tcqPNTJsb7R6BqSLo6) introducing linear systems concepts.         
